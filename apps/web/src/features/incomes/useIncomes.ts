...
/** How often an income repeats. `Once` creates no rule. */
export type IncomesRecurrence = "Once" | "Monthly" | "BiMonthly" | "Quarterly";

/** What the form sends when the user saves an income. */
export interface RegisterIncomesRequest {
...
export interface RegisteredIncomes {
...
/**
 * Loads the accounts an income can be received from.
 *
...
export function useRegisterIncome() {
  ...
  return useMutation({
    mutationFn: (request: RegisterIncomesRequest) =>
      apiPost<RegisteredIncomes>("/api/incomes", request),
