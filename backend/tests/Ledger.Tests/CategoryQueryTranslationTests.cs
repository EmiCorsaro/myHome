...
/// <summary>
/// Guards the duplicate-name lookup against the provider the application really runs on.
/// </summary>
/// <remarks>
/// The behaviour of the lookup is tested over SQLite (see <see cref="CategoryRegistrarTests"/>),
...
/// </remarks>
public sealed class CategoryQueryTranslationTests
{
    [Fact(DisplayName = "The duplicate-name lookup translates to SQL on PostgreSQL")]
    public void the_duplicate_name_lookup_translates_to_sql_on_postgresql()
    {
        ...
        var sql = context.Categories
            .Where(CategoryRegistrar.NameAlreadyTaken(LedgerFixtures.HouseholdId, "Ocio"))
            .ToQueryString();
